...
buildah run $bc_build pip3 install uvicorn fastapi pydantic
...
buildah run $bc pip3 install uvicorn fastapi pydantic
...
buildah config --cmd "python3 api.py --data_path=/bjorligames_react/data/" $bc
